...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text):
        nText = ''
        for letter in text:
            if letter in string.punctuation: nText += ' '
            else: nText += letter
        for w in nText.split():
            if w.lower() == self.word: return True
        return False
...
class PhraseTrigger(Trigger):
    def __init__(self, phrase):
        self.phrase = phrase
    def evaluate(self, story):
        return self.phrase in (story.getTitle()+story.getSubject()+story.getSummary())
...
def filterStories(stories, triggerList):
    ...
    for trigger in triggerList:
        for story in stories:
            if trigger.evaluate(story) == True and story not in filteredStories:
...
def main_thread(master):
    try:
        ...
        t1 = TitleTrigger("Statesman")
        t2 = SubjectTrigger("Statesman")
        t3 = PhraseTrigger("Statesman")
        t4 = OrTrigger(t2, t3)
        triggerlist = [t1, t4]
